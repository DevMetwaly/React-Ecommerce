...
class Cart extends React.Component{
    ...
    render(){
        ...
        const totalPrice = this.props.items.reduce((total, item) => total + (item.sale_price? item.sale_price : item.price) * item.cart.quantity, 0);
        const totalCommission = this.props.items.reduce((totalCom, item) => totalCom + item.commission * item.cart.quantity, 0);
                        ...
                        <Grid item xs={12} className={classes.root2}>
                            ...
                            <Grid container justify="center" className={classes.root3}>
                                ...
                                <Grid item md={4} xs={10} style={{padding:'0px 4px'}}>
                                    <CheckoutSummary totalCommission={totalCommission} totalPrice={totalPrice}/>    
