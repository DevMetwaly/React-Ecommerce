    ...
    traceCategory = (categories, id, trace=[]) => {
        if(id === 1){
            ...
            for(let i=0; i<trace.length; i++){
                ...
                // Capitalize first Letter --- need to handle if there are many words split by spaces.
                trace[i].name_en = trace[i].name_en.charAt(0).toUpperCase() + trace[i].name_en.slice(1).toLowerCase();
                traceArr.push({
                    id: trace[i].id,
                    ...
                    <Grid container justify='center' className={classes.root} spacing={2}>
                      ...
                      {this.state.categoryTrace?
                            <Grid item xs={11} className={classes.categoryTrace}>
                                ...
                                <Typography key={uuid()} className={classes.categoryLinkElement} variant="subtitle1">
                                    <Link
