...
const AddProduct1 = props => {
    ...
    const onSubmit = data => {
        ...
        if (childData === false) {
            message += 'select category and specs.';
            valid = false;
        }
        else
        ...
        if(description1.getPlainText('').length<50){
            message += 'Arabic description should be more than 50 characters.';
            valid = false;
        }
        ...
        if(description2.getPlainText('').length<50){
            message += 'English description should be more than 50 characters.';
            valid = false;
        }
        ...
        if(props.edit){
            productsAPI.post(`update/${props.edit}/`,formData, {headers: {'content-type': 'multipart/form-data'}})
            .then(res=>{
            ...
            productsAPI.post('', formData, {headers: {'content-type': 'multipart/form-data'}})
                .then(res => {
